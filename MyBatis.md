...
|        使用IDEA打开已创建的文件夹目录        |
| :------------------------------------------: |
| ![1621945941526](Pictures/1621945941526.png) |
...
|                选择Maven项目                 |
| :------------------------------------------: |
| ![1621945985140](Pictures/1621945985140.png) |
...
|                   GAV坐标                    |
| :------------------------------------------: |
| ![1621946226819](Pictures/1621946226819.png) |
...
<project xmlns="http://maven.apache.org/POM/4.0.0"
    ...
    <!--项目配置-->
    <groupId>com.banyuan.study</groupId>
    ...
    <environments default="MySqlDB">
        ...
        <environment id="MySqlDB">
            ...
            <dataSource type="org.apache.ibatis.datasource.pooled.PooledDataSourceFactory">
                ...
                <!-- 密码 -->
                <property name="password" value="hanchun123"/>
            </dataSource>
        </environment>
    </environments>
...
> 在resources目录中目录mapper，再mapper目录中创建UserMapper.xml文件
...
<mapper namespace="com.banyuan.study.dao.UserDao">
    ...
    <select id="selectUserById" resultType="com.banyuan.study.bean.User">
        <!--#{arg0} = 方法的第一个形参-->
...
jdbc.url=jdbc:mysql://localhost:3306/test?useUnicode=true&characterEncpding=utf8
jdbc.username=root
jdbc.password=hanchun123
```
...
where标签如果发现凭借的sql中第一个条件上面有and或者是or的话, 会帮我们去掉

如果发现where后面一个条件都没有, where关键字也会去掉
...
set标签如果发现后面的值多了 , 号, 会帮我们省略, 如果一个条件都没有,set关键字也去掉
...
2. 使用
   ...
   > 导包, 写mybatis的配置文件, 写映射文件
   >
   > 将映射文件和接口集合起来

3. 封装工具类

   > 将获取sqlsession ,提交, 关闭, 封装到一个类中,   重点看: 线程本地变量

4. mybatis传参

   > 一个javaBean , map集合, 散装参数
   >
   > 只有一个参 :  随便用
   >
   > 多个参数 : 
...
5. 动态sql 
   * sql : 包裹sql语句, 在别的地方引入 提供代码的复用性
   * if :  判断
   * where : 自动忽略where之后的一个and or
   * set : 自动忽略set之后多余的,号
   * trim : 可以代替where和set
   * foreach :  遍历  collection item open close  separator
...
7. mybatis注解操作
   ...
   > 在Mapper接口的方法上直接使用注解 写sql语句, 简单场景下可以使用